...
**Phase 13 插件口径验证（正确的评测方式：看任务能力，不看 loss）**：
冻结 Meta-ASH 30M（minimind 常规预训练，无任何针），NIAH 针测验
（未见过文档插入随机事实 [K1,K2,K3,V]，文末查询 [K1,K2,K3]）：
...
裸模型对未见针 rank 891/4436（彻底失明）；+DPSH 插件（对可见上下文的
精确后缀匹配，零训练）μ=0.3 即 92%，深度 60/128/196 全部深度不变。
**插件给冻结 30M 模型装上了它从未有过的精确回忆能力——这就是第一档的价值证明。**
